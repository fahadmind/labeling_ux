...
type InlineLabelMenuProps = ConnectedProps<typeof connector> &
  IInlineLabelMenuProps;

export const inlineLabelMenuHeight = 180;

export class InlineLabelMenu extends React.PureComponent<
  InlineLabelMenuProps,
  IInlineLabelMenuState
> {
  constructor(props) {
    super(props);
    this.state = {
  ...
  private prepareItems = () => {
    ...
    const items = fields
      .filter(
        (f) =>
          f.fieldType !== FieldType.Array && f.fieldType !== FieldType.Object
      )
    ...
    this.setState({
      items: items.length > 0 ? items : this.makeCreateFieldItems(),
    });
  };
  ...
  private makeCreateFieldItems = (): MenuItem[] => [
    {
      ...
      iconName: "GroupList",
      onClick: () =>
        this.handleCreateFieldClick(this.state.searchText, FieldType.String),
    },
    {
      text: "Select a subscription",
      iconName: "CheckboxComposite",
      onClick: () =>
        this.handleCreateFieldClick(
    ...
    },
    {
      ...
      onClick: () =>
        this.handleCreateFieldClick(this.state.searchText, FieldType.Signature),
    },
  ];
  ...
  public render() {
    const { showPopup, positionTop, positionLeft, hideInlineLabelMenu } =
      this.props;
